...
class LifeLone_MNIST(object):
    ...
    def build_model(self):
        ...
        # GAN networks
        gan_code = self.z
        G1 = Generator_Celeba_Tanh("GAN_generator1", gan_code, reuse=False)
        self.GenerationArr.append(G1)

        z_mean, z_log_sigma_sq = Encoder_Celeba2(self.inputs, inferenceName, batch_size=64, reuse=False)
        ...
        predict2,_ = MINI_Classifier(code, inferenceName2, reuse=False)
        p2 = self.gumbel_softmax(predict2,0.5)
        newCode = tf.concat([code,p2],1)

        reco = Generator_Celeba_Tanh("GAN_generator1", newCode, reuse=True)
        self.Reco = reco
        ...
        ## 1. GAN Loss
        # output of D for real images
        _, D_real_logits = Discriminator_Celeba_Orginal(self.inputs, "discriminator", reuse=False)

        # output of D for fake images
        _, D_fake_logits = Discriminator_Celeba_Orginal(G1, "discriminator", reuse=True)

        self.g_samples = G1

        self.d_loss = tf.reduce_mean(D_fake_logits) - tf.reduce_mean(D_real_logits)
        # generator loss
        ...
        self.d_loss = self.d_loss + 10 * self.gp_loss
        ...
        vae_vars = VAE_encoder_vars  + infer2
    ...
    def gradient_penalty(self):
        ...
        gradients = tf.gradients(Discriminator_Celeba_Orginal(interpolates,"discriminator", reuse=True), [interpolates])[0]
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for kk in range(taskCount):
                ...
                if kk > 0:
                    generated = self.GenerateByCount(20000)
