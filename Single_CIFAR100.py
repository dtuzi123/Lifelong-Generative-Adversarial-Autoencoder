...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.cifar_train_x = x_train
        self.cifar_train_label = y_train
        self.cifar_test_x = x_test
        self.cifar_test_label = y_test

        self.cifar_train_label = keras.utils.to_categorical(self.cifar_train_label, 100)
        self.cifar_test_label = keras.utils.to_categorical(self.cifar_test_label, 100)
        ...
        self.discrad_prediction = []
        self.discard_vaeloss = []
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        if self.componentCount == 0:
            ...
            gen_logits = Generator_SVHN_Tanh(GANGeneratorP, self.z, reuse=False)
            gen_logits = tf.reshape(gen_logits, (-1, 32, 32, 3))

            D_real_logits = Discriminator_SVHN_Logits(self.inputs, discStr, reuse=False)

            # output of D for fake images
            D_fake_logits = Discriminator_SVHN_Logits(gen_logits, discStr, reuse=True)


            my1 = Discriminator_SVHN_Logits(self.inputs, discStr, reuse=True)
            my2 = Discriminator_SVHN_Logits(self.gan_inputs, discStr, reuse=True)
            self.evaluatedLoss = -tf.reduce_mean(my1) + tf.reduce_mean(my2)
            ...
            disP = [var for var in T_vars if var.name.startswith(discStr)]
            ganP = [var for var in T_vars if var.name.startswith(GANGeneratorP)]
            ...
            with tf.variable_scope("foo", reuse=tf.AUTO_REUSE):
                ...
                self.d_optim1 = tf.train.RMSPropOptimizer(learning_rate=0.0001) \
                    .minimize(self.d_loss, var_list=disP)
                self.g_optim1 = tf.train.RMSPropOptimizer(learning_rate=0.0001) \
                    .minimize(self.g_loss, var_list=ganP)
    ...
    def build_model(self):
        ...
        self.input_x = tf.placeholder(tf.float32, [self.batch_size] + image_dims)
        ...
        self.gan_inputs = tf.placeholder(tf.float32, [self.batch_size] + image_dims)
        ...
        self.Wdistance = tf.reduce_mean(tf.reduce_sum(tf.square(self.inputs - self.input_x), [1, 2, 3]))
    ...
    def Calculate_Accuracy_ByALL_Batch(self, testX,testY, index):
        ...
        for i in range(totalBatchCount):
            ...
            pred = self.classifierModelArr[index].predict(batch)
            pred = np.argmax(pred,axis=1)
            for j in range(self.batch_size):
                myPro.append(pred[j])
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            train_gen = train_datagen.flow(self.cifar_train_x, self.cifar_train_label, batch_size=256)
            ...
            model_details = resnet18.fit(train_gen,
                                    steps_per_epoch=352,
                                      epochs=600,  # number of iterations
                                      verbose=1)
            a1,a2 = resnet18.evaluate(self.cifar_test_x,self.cifar_test_label)

            '''
            net.fit(self.cifar_train_x, self.cifar_train_label,
                verbose=1, epochs=350,
                validation_freq=1,
                    batch_size=128,
                shuffle=True)
            a1,a2 = net.evaluate(self.cifar_test_x,self.cifar_test_label)
            '''
            print(a1)
            print(a2)
