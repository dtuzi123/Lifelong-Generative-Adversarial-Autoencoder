...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        mnist_train_x, mnist_train_label, mnist_test, mnist_label_test = GiveMNIST32_Tanh()
        x_train, y_train, x_test, y_test = GiveFashion32_Tanh()
        ...
        with tf.Session(config=config) as sess:
            ...
            for t in range(taskCount):
                ...
                for i in range(epoch):
                    ...
                    a1 = (a1 + 1) * 127.5
                    cv2.imwrite(os.path.join("results/", "MyMNISTtoFashion" + str(i) + ".png"),
                                merge2(a1[:64], [8, 8]))
            ...
            reco = self.sess.run(self.stu_inferenceReco,feed_dict={self.inputs:batch1,self.d:taskLabe1,self.u:label1})

            batch1 = (batch1 + 1) * 127.5
            reco = (reco + 1) * 127.5
            cv2.imwrite(os.path.join("results/", "MNISTtoFashion_real"),
                        merge2(batch1[:64], [8, 8]))

            cv2.imwrite(os.path.join("results/", "MNISTtoFashion_reco"),
                        merge2(reco[:64], [8, 8]))
