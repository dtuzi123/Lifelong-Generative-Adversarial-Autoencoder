...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        mnist_train_x, mnist_train_label, mnist_test, mnist_label_test = GiveMNIST32()
        x_train, y_train, x_test, y_test = GiveFashion32()
        ...
        self.fashion_train_x = x_train
        self.fashion_train_label = y_train
        self.fashion_test_x = x_test
        self.fashion_test_label = y_test
    ...
    def Create_Teacher(self, index, selectedComponentIndex):
        ...
        gen_logits = Generator_SVHN(decoderName, newz, reuse=False)
        ...
        reco = Generator_SVHN(decoderName, newCode, reuse=True)
        ...
        VAE_parameters = encoderParamaters + infer1 + infer2
    ...
    def Create_StuClassifier(self, index, selectedComponentIndex):
        ...
        continous_variables1_sub = z_mean_sub + z_log_sigma_sub * tf.random_normal(tf.shape(z_mean_sub), 0, 1,
                                                                                   dtype=tf.float32)
        reco = Generator_SVHN(SuperGeneratorStr, continous_variables1_sub, batch_size=64, reuse=False)

        reco2 = Generator_SVHN(SuperGeneratorStr, self.z, batch_size=64, reuse=True)
        ...
        with tf.Session(config=config) as sess:
            ...
            for t in range(taskCount):
                if t == 0:
                    ...
                    setX = self.fashion_train_x
                    setY = self.fashion_train_label
                    taskSet = np.zeros((np.shape(setX)[0],2))
                    taskSet[:,1] = 1
